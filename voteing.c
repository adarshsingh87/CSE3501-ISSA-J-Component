...
void cast_vote(int * voter, int * candidate) {
  ...
  for (int j = 0; j < 6; j++) {
    ...
    if (darr[n - 1] == 1) {
      ...
      if (c == 1 || c == 2 || c == 3) {
        darr[n - 1] = 1;
        if (c == 1) voter[n - 1] = candidate[0];
        else if (c == 2) voter[n - 1] = candidate[1];
        else if (c == 3) voter[n - 1] = candidate[2];

        printf("Thank You for Voting!!! You're votes will stored until results are declared! \n");
        printf("\n\n\n\n\n\n\n\n\n");
      } else {
...
int main() {
  FILE * fp;
  fp = fopen("C:\\Users\\adars\\Desktop\\ISSA-j-comp\\1.txt", "w");
  int candidate[3];
  for (int i = 2; i >= 0; i--) {
  ...
  int v1, v2, v3;
  if (i == 5) {
    if (binary[0] == 1 && binary[1] == 1) v3 = 3;
    else if (binary[0] == 1 && binary[1] == 0) v3 = 1;
    else if (binary[0] == 0 && binary[1] == 1) v3 = 2;
    else v3 = 0;

    if (binary[3] == 1 && binary[2] == 1) v2 = 3;
    else if (binary[3] == 1 && binary[2] == 0) v2 = 2;
    else if (binary[3] == 0 && binary[2] == 1) v2 = 1;
    else v2 = 0;

    if (binary[4] == 1) v1 = 1;
    else if (binary[4] == 0) v1 = 0;
  ...
  if (i == 6) {
    ...
    if (binary[5] == 1 && binary[4] == 1) v1 = 3;
    else if (binary[5] == 1 && binary[4] == 0) v1 = 2;
    else if (binary[5] == 0 && binary[4] == 1) v1 = 1;
    else v1 = 0;
  ...
  if (strcmp(pass, password) == 0) {
    ...
    if (v1 == v2 && v2 == v3) printf("Clash occurred b/w candidate 1 and candidate 2 and candidate 3");
    else if (v1 > v2 && v1 > v3) printf("And the winner is CANDIDATE 1 (CONGRESS)!!!");
    else if (v2 > v1 && v2 > v3) printf("And the winner is CANDIDATE 2 (BJP)!!!");
    else
    if (v3 > v2 && v3 > v1)
